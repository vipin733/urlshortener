...
import { _loginUrl } from '../lib/baseurls'
...
const Login: NextPage = () => {
  ...
  const _submit = async (e: any) => {
    try {
      ...
      await axiosConfig.post(_loginUrl(), body)
      setLoader(false)
      Router.push("/")
    } catch (error: any) {
        ...
        <form className="mt-8 space-y-6" onSubmit={_submit}>
          <div className="-space-y-px rounded-md shadow-sm">
            <div>
              ...
              <input id="email-address" onChange={(e: any) => setEmail(e.target.value)} value={email} name="email" type="email" required className={_getClassOfInput(_getError("email", errors), false)} placeholder="Email address" />
            ...
            <div>
              ...
              <input id="password" name="password" value={password} onChange={(e: any) => setPassword(e.target.value)} type="password" minLength={6} required className={_getClassOfInput(_getError("password", errors), false)} placeholder="Password" />
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm">
              <div className="text-sm">
                {showLink ? <Link href="/register" className="font-medium text-indigo-600 hover:text-indigo-500" > <a><span>Dont have account?</span></a></Link> : null}
              </div>
            </div>
          </div>
